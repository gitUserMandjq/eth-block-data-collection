    ...
    public void endElement(String uri, String localName, String name) throws SAXException {
        ...
        if ("v".equals(name)) {
            ...
            if ("row".equals(name)) {
                ...
                if (curRow == 1) {
                    maxRef = ref;
                    totalColumnCount = cellList.size();
                }
                //补全一行尾部可能缺失的单元格
//                if (maxRef != null) {
...
//                    }
//                }
//                if(totalColumnCount != null && cellList.size() < totalColumnCount) {
//                	int len = totalColumnCount - cellList.size();
//                	for(int i = 0;i < len;i++) {
//                		cellList.add(curCol, "");
//                		curCol++;
//                	}
//                }
                if (flag && curRow != 1) { //该行不为空行且该行不是第一行，则发送（第一行为列名，不需要）
                    ...
                    excelReadDataDelegated.readExcelDate(sheetIndex, totalRowCount, totalColumnCount, curRow, cellList);
                    totalRows++;
                }
    ...
    public static void main(String[] args) {
		ExcelXlsxReaderWithDefaultHandler impl = new ExcelXlsxReaderWithDefaultHandler(null);
		System.out.println(impl.countNullCell("W1", "W2"));
	}
